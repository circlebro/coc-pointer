...
파일 이름이 서로 다른 것은 어느 범위를 검사하는지 이름에서 바로 보이게
하려는 것이다. 예전에는 이름이 같으면 저장소 전체를 한 번에 돌릴 때 수집이
멈추는 문제도 있었으나, 지금은 pyproject.toml 이 --import-mode=importlib 를
두어 pytest 가 파일 이름 대신 경로로 모듈을 구분한다.
"""
