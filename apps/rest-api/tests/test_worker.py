...
def test_서버가_명세를_따로_발행하지_않는다(client):
    """API 스펙은 api/openapi.yaml 한 벌뿐이다.

    FastAPI 가 코드를 훑어 만드는 명세는 스펙과 다르다. 응답 모양을 적지 않고
    operationId 도 다르다. 그것이 열려 있으면 누군가 거기에 코드 생성을 겨누어
    스펙이 두 벌이 된다. 열리지 않는 것을 여기서 지킨다.
    """
...
def test_표를_셀_때_D1_장부는_빼고_센다(client, fake_db):
    ...
    그것은 우리 표가 아니므로 개수에 섞이면 안내문과 어긋나 배포가 실패한
    것처럼 읽힌다. 가짜 D1 에는 그 표가 없어 저절로는 드러나지 않으므로
    여기서 일부러 만들어 둔다.
    """
    asyncio.run(
        fake_db.prepare(
    ...
    assert "clan_members" in tables  # 우리 표는 그대로 세어야 한다
