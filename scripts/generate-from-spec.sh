#!/usr/bin/env bash
# API 스펙에서 서버 모델과 프론트 타입을 생성한다.
#
#   ./scripts/generate-from-spec.sh
#
# 생성물은 손으로 고치지 않는다. 고치려면 api/openapi.yaml 을 고치고
# 이 스크립트를 다시 돌린다.
set -euo pipefail
cd "$(dirname "$0")/.."
...
uv run datamodel-codegen \
  ...
  --use-union-operator \
  --custom-file-header "# 이 파일은 api/openapi.yaml 에서 생성되었다. 손으로 고치지 마라."
echo "  → apps/rest-api/src/schemas.py"

echo

uv run ruff format apps/rest-api/src/schemas.py >/dev/null
