...
경계를 프론트와 백엔드 사이에만 긋는 것이 아니다. **앱은 서로를 남으로 대한다.** 언제든 하나를 떼어 따로 배포해도 나머지가 모르게 둔다.
...
`packages/core`는 앞쪽만 담는다. 점수 계산은 입력을 받아 숫자를 내놓을 뿐 아무것도 붙들지 않는다. 바깥과 만나는 자리는 전부 포트로 두어 각 앱이 자기 어댑터를 넣는다. 그래서 공유해도 결합이 생기지 않는다.

나중에 앱이 나뉘면 `packages/core`를 패키지로 배포하고 각자 버전을 고정해 쓴다. 지금은 한 저장소라 직접 참조할 뿐, 구조는 이미 그렇게 되어 있다.

**한 가지는 일부러 어긴다.** 서비스마다 데이터베이스를 따로 두지 않고 D1 하나를 함께 본다. 클랜원 50명의 자료를 나누고 그 사이를 맞추는 비용이 얻는 것보다 크다. 대신 **코드에서 경계를 지킨다.** 어느 서비스가 어느 표를 쓰는지 정해 두고, 서비스가 실제로 갈라질 때 다시 본다.
