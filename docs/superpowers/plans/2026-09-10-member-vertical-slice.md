...
`npm run dev` 를 띄우지 않는다. 멈추지 않는 프로세스이고, 화면을 눈으로 보는 일은 통제자가 따로 맡는다.

대신 빌드가 낸 것만 확인한다.

```bash
cd apps/web && ls dist/ && ls dist/assets/
```

기대: `index.html` 과 `assets/` 아래 `.js`·`.css` 파일이 있다.

```bash
grep -c "불러오지 못했습니다" dist/assets/*.js
```

기대: 1 이상. 실패했을 때 보여줄 문구가 번들에 실렸다는 뜻이다. 서버가 아직 배포되지 않았으므로 지금 화면을 열면 그 문구가 뜨는 것이 맞다.
